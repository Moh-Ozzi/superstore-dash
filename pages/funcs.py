...
############ "border-color": "#5F3DC4", "border-width": "1px", "border-style": "solid"
...
def create_map_graph(df, value):
    ...
    fig = px.choropleth(
        ...
        # labels={value: value},
                    ...
                    title=dict(font=dict(family='Arial', size=14), x=0.5), hoverlabel=dict(bgcolor="#f5813d"),
                    coloraxis_colorbar=dict(title=value_title))\
        .update_traces(marker_line_color='#B197FC', hovertemplate=f'<b>%{{hovertext}}</b><br><br>{value_title}: %{{z:,.0f}}')
